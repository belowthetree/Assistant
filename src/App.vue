...
import {
    emitModelResultEvent,
    listenModelUpdateEvent,
} from "./events/model_event";
    ...
    methods: {
        ...
        onInput() {
            ...
            const totalHeight = Math.min(
                this.maxHeight,
                inputHeight + this.conversationHeight + 35,
            );
            Window.setSize(new LogicalSize(300, totalHeight)).catch((e) =>
                console.log(e),
            );
        },
        ...
        updateConversationHeight() {
            ...
            this.modelOutputVisible =
                this.modelOutput && this.modelOutput.length > 0;
        },
        async commitCommand() {
    ...
    <main class="maincontainer rounded-lg" id="maincontainer">
        <div class="row macos-background rounded-lg flex flex-col justify-end">
            ...
            <textarea
                ...
                class="no-drag rounded-lg com-down"
                v-model="userInput"
                placeholder="输入你想说的话然后按下回车"
            ></textarea>
            <button class="right_bottom" @click="clickSetting">
                <i
                    class="iconBtn fa-solid fa-cog hover_color fa-5"
                    id="settingIcon"
                    :style="{ color: 'black' }"
                ></i>
            </button>
